      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="premium-shell mx-auto mb-[-46px] max-w-6xl rounded-lg" role="region" aria-label="Live provider status and code example">
          <div className="relative grid gap-px bg-white/[0.06] md:grid-cols-[0.95fr_1.3fr]">
            <div className="bg-card/95 p-5 sm:p-6 lg:p-7">
              <div className="flex items-center justify-between border-b border-white/[0.08] pb-4">
                <div>
                  ...
                  <p className="mt-1 text-lg font-semibold text-foreground">Live settlement path</p>
                </div>
                <span className="rounded-full border border-brand/25 bg-brand/10 px-3 py-1 text-xs font-bold text-brand">
                  online
                </span>
              </div>
              ...
              <div className="mt-6 space-y-3">
                {routes.map((route) => (
                  <div key={`${route.label}-${route.provider}`} className="rounded-md border border-white/[0.08] bg-surface/55 p-4">
              ...
              <div className="mt-5 grid grid-cols-3 gap-px overflow-hidden rounded-md border border-white/[0.08] bg-white/[0.08]">
                {[
                  ["2", "providers"],
                  ["0", "runtime deps"],
                  ["1", "typed API"],
                ].map(([value, label]) => (
                  <div key={label} className="bg-surface/70 p-3 text-left">
            ...
            <div className="bg-[#080b0e] p-5 sm:p-6 lg:p-7">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-oxide/80" />
                  <span className="h-2.5 w-2.5 rounded-full bg-amber/80" />
                  <span className="h-2.5 w-2.5 rounded-full bg-brand/80" />
                </div>
                <span className="font-mono text-xs text-muted">payments/index.ts</span>
              </div>

              <div className="overflow-x-auto">
                <pre className="min-w-[560px] text-left text-[13px] leading-7 sm:text-sm">
