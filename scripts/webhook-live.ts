 ...
 *      the Paystack / Flutterwave dashboard.
 ...
 *   3. bun run scripts/webhook-live.ts <paystack|flutterwave> <webhook-site-token-id>
 *
...
if (!provider || !token || !["paystack", "flutterwave"].includes(provider)) {
  console.log("Usage: bun run scripts/webhook-live.ts <paystack|flutterwave> <webhook-site-token-id>");
  process.exit(1);
}

const sigHeader = provider === "paystack" ? "x-paystack-signature" : "verif-hash";
