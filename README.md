...
> **Status: beta (pre-1.0).** pay-kit is fully typed, unit-tested (including webhook adapters against real Express/Fastify/Hono/NestJS servers, Node CJS/ESM consumer checks, and published-type validation), and **verified end to end against the live Paystack and Flutterwave sandboxes** - read flows, signature-verified webhooks, real paid charges (verify, saved-card re-charge, refund), and the full plan/subscription lifecycle, including a live failover drill. Live testing caught and fixed five real bugs (two Flutterwave redirect-URL omissions, a flat legacy webhook payload, and two subscription mappers). A few methods stay account-gated in test mode (payouts, `verifyTransfer`); the [Status](#status) section says exactly what is and isn't verified. Pin an exact version and test the flows you depend on - the API may still change before 1.0.
